...
"""
This script generates the read-heavy access pattern for ABDB vs Redis evaluation.
"""
import pandas as pd
import argparse
import numpy as np
...
parser.add_argument('mu', type=float, help='Expected value of the read count per sec')
parser.add_argument('sigma', type=float, help='Deviation of the read count per sec')
...
for i in range(args.function_count):
    ...
    read_count = int(np.random.normal(args.mu, args.sigma, 1)[0])
...
df.to_csv('read_heavy_access_pattern.csv',index=False)
print("DONE")
