...
for id in containers:
    ...
    delay = (int(container_name[-1:]) - 1) * 200
...
for id in all_containers:
    container_name = id
    print("\nStart GETs on docker container {} ...".format(container_name))
    container = client.containers.get(container_name)
    docker_ip_add = container.attrs['NetworkSettings']['IPAddress']

    kvs_container = client.containers.get(container_name)
    kvs_container.exec_run(
        "/KVS-CLIENT/bin/python3 /hydro/anna/client/python/max_get_client.py {} a".format(docker_ip_add), stderr=True,
        stdout=True)
...
fig.update_layout(
    ...
    yaxis_title="SUM accesses during a sec",
    font=dict(
...
now = datetime.now()
fig.write_html("test2_annaBellaDB_{}.html".format(now))
##############################################################################xxx

print("\nPLOT 'test2_annaBellaDB_{}.html' IS SAVED".format(now))
