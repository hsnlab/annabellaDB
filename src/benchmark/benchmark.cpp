...
void run(const unsigned &thread_id,
    ...
    string log_file = "log_benchmark_" + std::to_string(thread_id) + ".txt";
    string logger_name = "benchmark_log_" + std::to_string(thread_id);
    auto log = spdlog::basic_logger_mt(logger_name, log_file, true);
    log->flush_on(spdlog::level::info);
    log->info("Bencmark is ready on thread " + std::to_string(thread_id) + " and waiting for benchmark commands...");
  ...
  map<Key, std::pair<double, unsigned>> observed_latency;
    ...
    log->info("Wating pulling commands from tcp://*:" + std::to_string(thread_id + kBenchmarkCommandPort));
